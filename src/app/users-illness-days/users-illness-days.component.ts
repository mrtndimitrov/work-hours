...
@Component({
  selector: 'app-users-illness-days',
  templateUrl: './users-illness-days.component.html',
  styleUrls: ['./users-illness-days.component.scss']
})
export class UsersIllnessDaysComponent {
    ...
    this.organizationsService.getCurrentOrganization().then((organization: Organization) => {
      ...
      this.usersService.getUsers(this.organization.key).then(async (users: any) => {
        for (const user of users) {
          const vacationDays = await this.usersService.getIllnessDays(this.organization!.key, user.uid);
